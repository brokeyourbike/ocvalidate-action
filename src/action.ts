...
export async function run(): Promise<void> {
  try {
    ...
    let ocvalidatePath: string = tc.find(
      utils.CACHE_KEY,
      utils.getFullVersion(opencore),
    );
    if (ocvalidatePath !== '') {
    ...
    const message =
      err instanceof Error ? err.message : 'Failed to install ocvalidate';
...
async function findLatestRelease(
...
): Promise<utils.IOpenCoreRelease> {
  let opencore: utils.IOpenCoreRelease;
  ...
  for (const commitSha of commitsSha) {
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Failed to find latest release';
      core.debug(message);
    }
  }

...
export async function download(
  opencore: IOpenCoreRelease,
  platform: string,
): Promise<IOcvalidateVersionFile> {
  try {
    core.info(
      `Acquiring OpenCore ${opencore.version} from ${opencore.downloadUrl}`,
    );
    ...
    for (const filePath of await globber.glob()) {
      ...
      const file: IOcvalidateVersionFile = {
        version: getFullVersion(opencore),
        path: filePath,
      };
    ...
    throw new Error(
      `Failed to download ocvalidate from OpenCore ${opencore.version}: ${errMessage}`,
    );
  }
}
...
export async function cache(file: IOcvalidateVersionFile): Promise<string> {
